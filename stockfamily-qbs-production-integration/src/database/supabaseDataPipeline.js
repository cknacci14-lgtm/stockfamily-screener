...
if (!supabaseUrl || !supabaseKey) {
    throw new Error("SUPABASE_URL dan SUPABASE_SERVICE_ROLE_KEY wajib tersedia untuk production pipeline.");
}
const supabase = createClient(supabaseUrl, supabaseKey);

async function fetchHistoricalDataFromSupabase(options) {
    const opts = options || {};
    const limitDays = Number.isFinite(Number(opts.limitDays)) ? Number(opts.limitDays) : 5000;
            ...
            while(hasMore){
                ...
                (res.data||[]).forEach(row=>{
                    ...
                    if(!formattedDb[symbol]) formattedDb[symbol]=[];
                                const foreignBuy = row.foreign_buy == null ? null : Number(row.foreign_buy);
                    const foreignSell = row.foreign_sell == null ? null : Number(row.foreign_sell);
                    const foreignNet = Number.isFinite(foreignBuy) && Number.isFinite(foreignSell) ? foreignBuy - foreignSell : null;
                    formattedDb[symbol].push({
                        ...
                        bidVolume: row.bid_volume == null ? null : Number(row.bid_volume),
                        offer: row.offer == null ? null : Number(row.offer),
                        offerVolume: row.offer_volume == null ? null : Number(row.offer_volume)
                    });
                });
