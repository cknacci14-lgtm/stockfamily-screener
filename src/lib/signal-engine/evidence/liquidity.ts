...
export function evaluateLiquidity(
  ...
  if (!validValue && !validFrequency && !validVolume) {
    return {
      ...
      explanation:
        "Insufficient liquidity data to assess tradability.",
    };
  }
  ...
  const valueHealthy =
    validValue && value! >= 10_000_000_000;

  const frequencyHealthy =
    validFrequency && frequency! >= 1_500;
  ...
  if (
    ...
    return {
      ...
      explanation:
        "Trading value, frequency, volume, and available history meet the minimum execution-quality baseline.",
    };
  }
  ...
  if (
    (validValue && value! < 2_000_000_000) ||
    ...
    return {
      ...
      explanation:
        "Trading activity is materially below the minimum liquidity baseline.",
    };
  }
  ...
  return {
    ...
    explanation:
      "One or more execution-quality measures are below the preferred baseline.",
  };
}