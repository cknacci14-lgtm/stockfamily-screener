...
async function runQbsProductionSnapshot({ force = false, targetDate = null } = {}) {
  const spec = readJson(SPEC_FILE);

  const requestedDate = targetDate ? String(targetDate).trim() : null;
  ...
  if (requestedDate) {
    const available = Object.values(database).some((history) =>
      Array.isArray(history) && history.some((row) => String(row.date || row.trade_date || "").slice(0, 10) === requestedDate)
    );
    if (!available) {
      throw new Error(`QBS production blocked: targetDate ${requestedDate} is not available in historical database.`);
    }
  }
